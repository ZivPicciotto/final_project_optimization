...
def plot_flow_network(solution, costs, a, b):
    ...
    # Outlet positions (right side)
    for j in range(12):
        pos[f'R{j+1}'] = (2, 1.8 - j*0.15)
    ...
    # Draw flows with width proportional to shipment amount
    ...
    for i in range(2):
        for j in range(12):
            ...
            if flow > 0.1:  # Only show meaningful flows
                ...
                plt.annotate("", 
                            xy=pos[f'R{j+1}'], xytext=pos[f'F{i+1}'],
                            arrowprops=dict(arrowstyle="->", 
                                          linewidth=width,
                                          color='green' if i==0 else 'purple',
                                          alpha=0.7))
                mid_x = (pos[f'F{i+1}'][0] + pos[f'R{j+1}'][0])/2
                mid_y = (pos[f'F{i+1}'][1] + pos[f'R{j+1}'][1])/2
                plt.text(mid_x, mid_y, f"{flow:.1f}\n(${costs[i,j]}/unit)",
    ...
    plt.axis('off')
    plt.tight_layout()
    plt.savefig('transport_network.png', dpi=300)
    plt.close()
